...
def interactive_get_possible_split(state):
    ...
    split = lambda total: [
            f"{i}-{total-i}" for i in range(1, total) if 0 < i < NHANDS and 0 < total-i < NHANDS]
    a,b,c,d,t = state
    if t == 0:
        return split(a+b)
    return split(c+d)
...
def game_player_turn(state, visited):
    ...
    if i == '1':  # Attack
        ...
        assert reduce_state(nstate) in rpos, "Invalid move"
        a, b = i.split("-")
...
def game_prog():
    ...
    while True:
        ...
        if turn == 1:  # Pat's turn
            ...
            pat_sc, rstt = pat(rstt, 10, vis)
            #print("Pat's confidence:", pat_sc/1.5)
